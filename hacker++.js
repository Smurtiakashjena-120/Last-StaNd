...
//acessing surviver camera

const survCanva = document.querySelector("#survCam");

survCanva.height = canvasHeight / 3 ;
survCanva.width = canvasWidth / 5 ;

const c2 = survCanva.getContext("2d");

//end of survcam acess 
...
startGame.addEventListener("click" , () => {
    ...
    autoFire.classList.remove("hidden")
    jetBtn.classList.remove("hidden")
})
...
function showHealth(){
     ...
     for(let i=0;i<10;i++){
      let color = i < 4 ? "orange" : "lightgreen"
...
//clouds
 ...
 c.drawImage(treeImage,canvasWidth - 90,yTree, treeSelf.width,treeSelf.height);
  ...
  c2.drawImage(lapImage,(canvasWidth/2 -100) / 5,(canvasHeight - self.height -10) / 3 - 7 , 50,60 );
...
// assigning to update survivor cam
      this.image = handlerArray[frame]  
...
this.draw=function(){
    c.drawImage(image, self.x, self.y, self.width, self.height);
    //adding to draw in cam
    c2.drawImage(image, self.x /5, self.y /3 + 5 , self.width /5, self.height /4    );
}
this.update=function(){
...
function collisionCheckBullet(bullet, jombie, jombieIndex, bulletIndex,array) {
...
function animate() { 
    ...
    c2.clearRect(0, 0, survCanva.width, survCanva.height);
...
blockArray.forEach(block => {
    ...
    jombieArray.forEach((jombie,jombieIndex) => {
        ...
        collisionCheckBullet(bullet,jombie,jombieIndex,bulletIndex,bulletArray)
        })
    })
...
//landmine update
landMineArray.forEach(landmine => {
    landmine.update();
})
//trap update
trapArray.forEach(landmine => {
    landmine.update();
})

jombieArray.forEach((jombie,jombieIndex) => {
    landMineArray.forEach((landmine,landmineIndex)=>{

        jombieOnLandmine(jombie,jombieIndex,landmine,landmineIndex)
        
    })
})
jombieArray.forEach((jombie,jombieIndex) => {
    trapArray.forEach((trap,trapIndex)=>{

        jombieOnTrap(jombie,jombieIndex,trap,trapIndex)
...
jombieArray.forEach(jombie => {
if(jombie.autoBullet) return;
let newBullet = new AutomatedBullet(jombie);
jombie.autoBullet = newBullet;
automatedBulletArray.push(newBullet);
})

//automated defense system
if(autoDefenceFlag){
    automatedBulletArray.forEach(bullet => {
        bullet.update();
    })
    
    
    jombieArray.forEach((jombie,jombieIndex) => {
        automatedBulletArray.forEach((bullet,bulletIndex) => {
            collisionCheckBullet(bullet, jombie, jombieIndex, bulletIndex,automatedBulletArray)
        })
    })
    automatedBulletArray.forEach((bullet, index) => {
        if (bullet.y > canvas.height) {
            bulletArray.splice(index, 1);
        }
    });
}else{
  automatedBulletArray = [];
}
...
inventoryBtn.addEventListener("click",inventoryShow)
...
const landMineArray = []
let landMineHeight =14;
let landMineWidth =50;
...
const landMine = document.querySelector("#landMine")
const trap = document.querySelector("#trap")



landMine.addEventListener("click" , addLandmine)
trap.addEventListener("click" , addTrap)


function addLandmine(){

let x = bheem.x;
let y =bheem.y + bheem.height - landMineHeight

let newLandmine = new Landmine(x,y);
landMineArray.push(newLandmine);


bheem.y = newLandmine.y - bheem.height;
...
let x = bheem.x;
let y =bheem.y + bheem.height - landMineHeight
let newTrap = new Trap(x,y);
trapArray.push(newTrap);


bheem.y = newTrap.y - bheem.height;

}


function jombieOnLandmine(jombie,jombieIndex,landmine,landmineIndex){
    
      
if(jombie.x + jombie.width > landmine.x && jombie.x < landmine.x + landmine.width &&   landmine.y < jombie.y + jombie.height &&
    landmine.y + landmine.height > jombie.y){

    for(let i=0 ; i< 20 ; i++){
        let newParticle = new Particle(landmine.x + landmine.width , landmine.y ,jombie.color)
        parcticleArray.push(newParticle);
    } 
    //updating scoree   
    Score += 10;
    jombieCount++;
    jombieArray.splice(jombieIndex, 1);
    landMineArray.splice(landmineIndex, 1);

}


}
function jombieOnTrap(jombie,jombieIndex,trap,trapIndex){
    
      
if(jombie.x + jombie.width > trap.x && jombie.x < trap.x + trap.width &&   trap.y < jombie.y + jombie.height &&
    trap.y + trap.height > jombie.y){

        trap.y = 12;
        jombie.y = -jombie.height + 20 ;
        jombie.x =trap.x
        jombie.velocity.x =0;
        jombie.gravity = 0;
...
//acessing jet count button
const jetBtn = document.querySelector("#jetPack");



//maximum time a player can use a jet pack is two
let maxJetUse = 0;
...
//event listner i=on jet pack
jetBtn.addEventListener("click",(event)=>{
    if(countId) return;
    maxJetUse++;
    if(maxJetUse > 2){
        alert("You Have Used JetPack 2 Times !!");
        return;
    }
    jetPack(event);
    handleCountId(event);
...
if(countForJet > 7){
    ...
    //canceling previous frame
    cancelAnimationFrame(jetId)


        //declaring position of image and its width
    this.x = bheem.x - 5 ;
    this.y = bheem.y + bheem.height -5
    this.width = bheem.width -10;
    this.height =20
     c.drawImage(image, this.x, this.y, this.width, this.height);
    if(bheem.y >  canvas.height / 3){
        bheem.velocity.y = -1;
     
     }else{
         bheem.velocity.y =0 
     }
     jetId=requestAnimationFrame(jetPack);
}
...
//automated defense
const autoFire =document.querySelector("#autoFire");

//declaring a variable to start automated defense;

let autoDefenceFlag =false;


autoFire.addEventListener("click",()=>{
    autoDefenceFlag = !autoDefenceFlag;
    autoFire.innerText = autoDefenceFlag ? "Stop Fire" : "Auto Fire"
})


function AutomatedGun() {
    ...
    // Load the image
    const image = new Image();
    image.src = './assets/defenseGun.png';
    const self = this;
    ...
    this.draw = function() {
        c.drawImage(image, self.x, self.y, self.width, self.height);
        c.drawImage(image, canvas.width - 40, self.y, self.width, self.height);
    };
...
// variabe to store automated bullets

let automatedBulletArray = [];

function AutomatedBullet(jombie) {
...
// declaring for survive rcam
function CamSurviver() {
    this.width = 20;    
    this.height = 40;
    this.x = survCanva.width / 2 - 10;
    this.y = survCanva.height - this.height;
    this.flipped = false
    ...
    this.draw = function(image) {
//write focus word on board
        c2.font = '10px Arial'; 
    c2.textAlign = 'left';
    c2.textBaseline = 'middle'; 
    c2.fillStyle = 'white';
    c2.fillText(`FOCUS ! `, 10, 20);
        if (image && image.complete){ 
                c2.save();
        if (this.flipped) {
            // Flip the image horizontally
            c2.translate(this.x + this.width / 2, this.y + this.height / 2);
            c2.scale(-1, 1);
            c2.drawImage(image, -this.width / 2, -this.height / 2, this.width, this.height);
        } else {
            c2.drawImage(image,
                this.x,
                this.y,
